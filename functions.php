...
function load_sec_css() {
    ...
    $response = wp_remote_get('https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css');
...
function load_js() {
    ...
    $response = wp_remote_get('https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js');
    $code = wp_remote_retrieve_response_code( $response );
    if ( !is_wp_error( $response ) ){
        if( isset( $url ) && !empty( $url) && ( $code == '200') ){
            wp_register_script( 'poper', $url, array(), ' ' );
            wp_enqueue_script( 'poper' );
        }
    }
    else{
        wp_register_script( 'poper', get_template_directory_uri() . '/js/bootstrap.bundle.min.js', array(), ' ' );
        wp_enqueue_script( 'poper' );
    }
    $url = 'https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js';
    $response = wp_remote_get('https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js');
    $code = wp_remote_retrieve_response_code( $response );
    if ( !is_wp_error( $response ) ){
        if( isset( $url ) && !empty( $url) && ( $code == '200') ){
            wp_register_script( 'b4', $url, array(), ' ' );
            wp_enqueue_script( 'b4' );
        }
    }
    else{
        wp_register_script( 'b4', get_template_directory_uri() . '/js/bootstrap.min.js', array(), ' ' );
        wp_enqueue_script( 'b4' );
    }
...
if(isset($user_email)&&!empty($user_email)) {     
   $message .=   '<tr style="height:30px;width:auto;border-bottom: 1px solid black;">
    ...
    if( (isset($user_name)&&!empty($user_name)) || (isset($user_phone)&&!empty($user_phone)) || (isset($user_email)&&!empty($user_email)) ){
